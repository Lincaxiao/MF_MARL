...
class MFAC_Trainer:
    def __init__(self, env, config):
        ...
        # ========= ① Actor 和 Critic 初始化 =========
        # Actor：不共享，为每个 agent 创建独立的 Actor
        self.user_actors = [HierarchicalActor(self.user_obs_dim, self.mean_field_dim, self.n_servers).to(self.device) for _ in range(self.n_users)]
        self.server_actors = [Actor(self.server_obs_dim, self.mean_field_dim, self.server_action_dim).to(self.device) for _ in range(self.n_servers)]
        # --- Critic 共享参数 ---
        # 所有用户共享同一个 Critic；所有服务器共享同一个 Critic
        self.shared_user_critic = Critic(self.user_obs_dim, self.mean_field_dim, self.user_action_dim).to(self.device)
        self.shared_server_critic = Critic(self.server_obs_dim, self.mean_field_dim, self.server_action_dim).to(self.device)

        # 为了保持后续索引逻辑不变，构造重复引用列表，但请注意这些引用指向同一网络
        self.user_critics = [self.shared_user_critic] * self.n_users
        self.server_critics = [self.shared_server_critic] * self.n_servers

        # （已在上方共享并编译）

        # 收集所有独立 Actor 的参数
        all_actor_params = [p for actor in self.user_actors for p in actor.parameters()] + \
                           [p for actor in self.server_actors for p in actor.parameters()]
        # 仅收集一次共享 critic 的参数，避免重复
        all_critic_params = list(self.shared_user_critic.parameters()) + list(self.shared_server_critic.parameters())
    ...
    def _prefill_buffer(self):
        """用随机策略填充回放缓冲区到 buffer_capacity"""
        logging.info(f"Prefilling replay buffer with random actions until size {self.config['buffer_capacity']}...")
        obs = self.env.reset()
        self.smooth_mean_field = self.env.get_mean_field_state()

        while len(self.buffer) < self.config['buffer_capacity']:
            true_mean_field = self.env.get_mean_field_state()
            self.smooth_mean_field = (1 - self.config['alpha']) * self.smooth_mean_field + self.config['alpha'] * true_mean_field
            ...
            for i in range(self.n_agents):
                ...
                if len(valid_actions) > 0:
                    ...
                    action = 0  # Fallback, 尽管mask的设计保证至少有一个有效动作
        ...
        logging.info(f"Replay buffer prefilled with {len(self.buffer)} experiences.")

    def train(self):
        self._prefill_buffer()  # 在训练前调用预填充
        for episode in range(self.config['num_episodes']):
            ...
            for t in range(self.config['episode_length']):
                ...
                # 2. 动作选择 (独立 Actor, 逐个推理)
                actions = []
                action_masks = self.env.get_agent_action_mask()
                with torch.no_grad():
                    mf_tensor = torch.tensor(self.smooth_mean_field, dtype=torch.float32, device=self.device).unsqueeze(0)

                    # ---- 用户动作 ----
                    for i in range(self.n_users):
                        agent_obs = torch.tensor(obs[i], dtype=torch.float32, device=self.device).unsqueeze(0)
                        agent_mask = torch.tensor(action_masks[i], dtype=torch.bool, device=self.device).unsqueeze(0)
                        
                        logits = self.user_actors[i](agent_obs, mf_tensor)
                        logits[agent_mask == 0] = -1e10
                    ...
                    for i in range(self.n_servers):
                        ...
                        agent_obs = torch.tensor(obs[agent_idx], dtype=torch.float32, device=self.device).unsqueeze(0)
                        agent_mask = torch.tensor(action_masks[agent_idx], dtype=torch.bool, device=self.device).unsqueeze(0)

                        logits = self.server_actors[i](agent_obs, mf_tensor)
                        logits[agent_mask == 0] = -1e10
                        action = torch.distributions.Categorical(logits=logits).sample().item()
                        actions.append(action)
                # 3. 与环境交互
    ...
    def update_models(self):
        ...
        for i in range(self.n_agents):
            ...
            # print(f"entropy: {entropy.item()}, actor_loss: {actor_loss.item()}")
        ...
        # 仅对共享 critic 参数裁剪
        torch.nn.utils.clip_grad_norm_(list(self.shared_user_critic.parameters()) + list(self.shared_server_critic.parameters()), 1.0)
