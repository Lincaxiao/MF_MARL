...
class EdgeBatchEnv:
    def __init__(self, n_users: int, n_servers: int, batch_proc_time: dict, gmm_config: dict = None,
        ...
        if isinstance(batch_proc_time, list):
            ...
            self.batch_proc_time_list = [ {int(k): v for k, v in cfg.items()} for cfg in batch_proc_time ]
        elif isinstance(batch_proc_time, dict):
            self.batch_proc_time_list = [ {int(k): v for k, v in batch_proc_time.items()} for _ in range(self.n_servers) ]
        else:
            raise TypeError("batch_proc_time 应为 dict 或 list[dict]")

        # 兼容旧属性（第0台服务器配置）
        self.batch_proc_time_map = self.batch_proc_time_list[0]
    ...
    def _calculate_batch_time(self, batch_size: int, server_id: int) -> int:
        ...
        cfg = self.batch_proc_time_list[server_id]
        sorted_thresholds = sorted(cfg.keys())
        ...
        if sorted_thresholds:
            return cfg[sorted_thresholds[-1]]
