...
class EdgeBatchEnv:
    def __init__(self, n_users: int, n_servers: int, batch_proc_time: dict, gmm_config: dict = None,
        ...
        # 将传入的字典的键转换为整数并排序，以便查找
        self.batch_proc_time_map = {int(k): v for k, v in batch_proc_time.items()}
        self.sorted_batch_sizes = sorted(self.batch_proc_time_map.keys())
        ...
        self.gmm_models = {
            ...
            'server_b_size': None,
            'server_t': None,
            'server_idle_time': None, # modified
        }
        ...
        self.history_buffer = {
            ...
            'server_idle_time': collections.deque(maxlen=buf_len), # modified
        }
    ...
    def collect_data_for_gmm(self):
        ...
        server_idle_times = [s['idle_time'] for s in self.servers_state] # modified
        ...
        self.history_buffer['server_idle_time'].extend(server_idle_times) # modified
    ...
    def reset(self):
        ...
        self.servers_state = [{'q': collections.deque(), 'b': [], 't': -1, 'idle_time': 0} for _ in range(self.n_servers)] # modified
    ...
    def _calculate_batch_time(self, batch_size: int) -> int:
        ...
        # 寻找第一个大于或等于 batch_size 的批次
        for size in self.sorted_batch_sizes:
            if batch_size <= size:
                return self.batch_proc_time_map[size]
        
        # 如果没有找到（即 batch_size 大于所有已定义的批次），则使用最大批次的处理时间
        if self.sorted_batch_sizes:
            max_size = self.sorted_batch_sizes[-1]
            return self.batch_proc_time_map[max_size]
    ...
    def _get_server_obs(self, server_id: int):
        server = self.servers_state[server_id]
        return [len(server['q']), len(server['b']), server['t'], server['idle_time']] # modified
    ...
    def get_mean_field_state(self):
        ...
        if self.n_servers > 0:
            ...
            # 服务器空闲时间分布的后验概率 # modified
            idle_time_features = self._predict_with_gmm(server_idle_times, "server_idle_time") # modified
            mf_features.append(idle_time_features) # modified
