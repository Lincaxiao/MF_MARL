...
from RealEdgeBatchEnv import EdgeBatchEnv # modified
...
def setup_logger(log_dir):
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler(log_file, mode='a'), logging.StreamHandler()], # modified: use append mode
    )
...
class MAPPO_MF_Trainer:
    ...
    def save_checkpoint(self, filepath):
        """保存模型和GMM状态"""
        checkpoint = {
            'user_actor_state_dict': self.user_actor.state_dict(),
            'server_actor_state_dict': self.server_actor.state_dict(),
            'gmm_models': self.env.gmm_models,
            'gmm_history_buffer': self.env.history_buffer
        }
        if self.use_global:
            checkpoint['global_critic_state_dict'] = self.global_critic.state_dict()
        else:
            checkpoint['user_critic_state_dict'] = self.user_critic.state_dict()
            checkpoint['server_critic_state_dict'] = self.server_critic.state_dict()
    ...
    def load_checkpoint(self, filepath):
        ...
        self.user_actor.load_state_dict(checkpoint['user_actor_state_dict'])
        self.server_actor.load_state_dict(checkpoint['server_actor_state_dict'])
        
        if self.use_global:
            self.global_critic.load_state_dict(checkpoint['global_critic_state_dict'])
        else:
            self.user_critic.load_state_dict(checkpoint['user_critic_state_dict'])
            self.server_critic.load_state_dict(checkpoint['server_critic_state_dict'])
            
        self.env.gmm_models = checkpoint['gmm_models']
        self.env.history_buffer = checkpoint['gmm_history_buffer']
    ...
    def _run_training_loop(self, num_episodes, start_episode=0):
        """内部训练循环"""
        for ep in range(start_episode, start_episode + num_episodes):
            ...
            total_episodes = self.config['episodes_phase1'] + self.config['episodes_phase2']
    ...
    def train_phase1(self):
        ...
        self._warmup_critic()
        self._run_training_loop(self.config['episodes_phase1'])
        self.save_checkpoint(self.config['checkpoint_path'])
        logging.info("--- Training Phase 1 Finished ---")

    def train_phase2(self):
        """第二阶段训练"""
        logging.info("--- Starting Training Phase 2 ---")
        self.load_checkpoint(self.config['checkpoint_path'], weight_only=True)
        # 注意：第二阶段不需要重新收集初始GMM数据或预热，因为我们加载了之前的状态
        start_episode = self.config['episodes_phase1']
        self._run_training_loop(self.config['episodes_phase2'], start_episode=start_episode)
        logging.info("--- Training Phase 2 Finished ---")


if __name__ == '__main__':
    PHASE_TO_RUN = 1  # 在这里切换: 1 或 2
    ...
    config = {
        ...
        'log_dir': '', # 占位，根据参数自动生成
        'checkpoint_path': '', # 占位，根据参数自动生成
    }

    # --- 根据配置动态生成日志和检查点路径 ---
    mf_tag = 'MF' if config.get('use_mean_field', True) else 'NoMF'
    global_tag = 'Global' if config.get('use_global', True) else 'Local'
    suffix = (f"{config['n_users_phase1']}_{config['n_servers']}_to_"
              f"{config['n_users_phase2']}_{config['n_servers']}_"
              f"{global_tag}_{mf_tag}")
    
    config['log_dir'] = f"logs/Trans_mappo_mf_{suffix}"
    config['checkpoint_path'] = os.path.join(config['log_dir'], 'checkpoint.pth')
    ...
    if PHASE_TO_RUN == 1:
        logging.info("--- Setting up for Phase 1 ---")
        n_users_current = config['n_users_phase1']
    elif PHASE_TO_RUN == 2:
        logging.info("--- Setting up for Phase 2 ---")
        n_users_current = config['n_users_phase2']
    else:
        raise ValueError(f"Invalid PHASE_TO_RUN: {PHASE_TO_RUN}")
