...
class EdgeBatchEnv:
    def __init__(self, n_users: int, n_servers: int, batch_proc_time: dict, gmm_config: dict = None,
        ...
        # ------- 批处理时间配置 -------
        # 允许传入单个 dict（所有服务器共享）或 list[dict]（每台服务器独立）
    ...
    def _calculate_batch_time(self, batch_size: int, server_id: int) -> int:
        ...
        # 如果 batch_size 大于所有已定义的阈值，则使用最大阈值对应的时间
    ...
    def _execute_server_action(self, server_id: int, action: int):
        ...
        # 当服务器空闲、队列中有任务，并且 (动作为1 或 队列长度达到最大批次) 时，启动批处理
        if server['t'] == -1 and len(server['q']) > 0 and (action == 1 or len(server['q']) >= self.max_batch_size):
